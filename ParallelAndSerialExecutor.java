...
public class ParallelAndSerialExecutor<T> {
     ...
     */
    private interface ParallelRunnable extends Runnable {
        /**
         * Used for testing only
         */
     ...
     * Used for testing only
     */
    private static SerialRunnable<String> createStringSerialRunnable(final String identifier, final Runnable runnable) {
        return new SerialRunnable<String>() {
            @Override
    ...
    public static void testEverything() {
        final ParallelAndSerialExecutor<Story> testExecutor = new ParallelAndSerialExecutor<>(4, 10, 10);

        //PARALELL
        WPThreadPool.execute(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < 100; i++) {
                    String id = "Parallell:" + i;
                    ...
                    testExecutor.execute(new ParallelAndSerialExecutor.ParallelRunnable() {
                        @Override
                        public void run() {
                            try {
                            ...
                            Logger.d("ParalellJobsBro", "finished executing " + story.getId());

                            //TO SEE ALL TESTS
                            Logger.d("ParallelAndSerialExecutor", "finished executing " + story.getId());
                        }
        ...
        WPThreadPool.execute(new Runnable() {
            ...
            public void run() {
                for (int i = 0; i < 10; i++) {
                    String id = "Serial:" + 1337;
                    int insertionNumber = i;

                    final Story story = new Story();
                    story.setId(id);
                    story.setTitle(id + " ==> " + insertionNumber);
                    testExecutor.execute(ParallelAndSerialExecutor.createStringSerialRunnable(id, new Runnable() {
                        @Override
                        public void run() {
                            try {
                                Thread.sleep(new Random().nextInt(2000));
                            ...
                            Logger.d("SerialJobsBro", "finished executing " + story.getTitle());

                            //TO SEE ALL TESTS
                            Logger.d("ParallelAndSerialExecutor", "finished executing " + story.getTitle());
                        }
                    }));
                }
            }
                ...
                for (int i = 0; i < 50; i++) {
                    ...
                    testExecutor.execute(ParallelAndSerialExecutor.createStringSerialRunnable(id, new Runnable() {
                        ...
                        public void run() {
                            ...
                            Logger.d("SerialJobsFoo", "finished executing " + story.getTitle());
                ...
                for (int i = 0; i < 100; i++) {
                    ...
                    testExecutor.execute(ParallelAndSerialExecutor.createStringSerialRunnable(id, new Runnable() {
                        ...
                        public void run() {
                            ...
                            Logger.d("SerialJobsBar", "finished executing " + story.getTitle());
